...
from chunkformer.dataset import processor
from chunkformer.dataset.datapipes import WenetRawDatasetSource, WenetTarShardDatasetSource
...
def Dataset(
    ...
    if list_shuffle:
        ...
        list_shuffle_size = list_shuffle_conf.get("shuffle_size", list_shuffle_size)
    if data_type == "raw":
        dataset = WenetRawDatasetSource(
            data_list_file,
            partition=partition,
    ...
    if feats_type == "fbank":
        ...
        dataset = dataset.map(
            partial(processor.compute_log_mel_spectrogram, **log_mel_spectrogram_conf)
        )
