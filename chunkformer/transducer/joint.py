...
class TransducerJoint(torch.nn.Module):
    ...
    def forward(
        self, enc_out: torch.Tensor, pred_out: torch.Tensor, pre_project: bool = True
    ) -> torch.Tensor:
        """
        Args:
        ...
        if not self.hat_joint and self.ffn_out is not None:
            ...
            label_logp = self.token_pred(out).log_softmax(dim=-1)  # [B,T,U,vocab-1]
            # scale token logp
