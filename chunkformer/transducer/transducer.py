...
from chunkformer.modules.decoder import BiTransformerDecoder, TransformerDecoder
from chunkformer.modules.label_smoothing_loss import LabelSmoothingLoss
from chunkformer.transducer.predictor import PredictorBase
from chunkformer.transducer.search.greedy_search import basic_greedy_search
from chunkformer.transducer.search.prefix_beam_search import PrefixBeamSearch
...
class Transducer(ASRModel):
    ...
    def __init__(
        ...
        joint: nn.Module,
        attention_decoder: Optional[Union[TransformerDecoder, BiTransformerDecoder]] = None,
        ...
        if self.enable_k2:
            self.simple_am_proj = torch.nn.Linear(self.encoder.output_size(), vocab_size)
            self.simple_lm_proj = torch.nn.Linear(self.predictor.output_size(), vocab_size)
    ...
    def forward(
        ...
        loss_rnnt = self._compute_loss(
            encoder_out, encoder_out_lens, encoder_mask, text, text_lengths, steps=steps
        )
    ...
    def _cal_transducer_score(
        ...
        loss_td = torchaudio.functional.rnnt_loss(
            joint_out, rnnt_text, xs_in_lens, hyps_lens.int(), blank=self.blank, reduction="none"
        )
        return loss_td * -1
    ...
    def _cal_attn_score(
        ...
        decoder_out, r_decoder_out, _ = self.decoder(
            encoder_out, encoder_mask, hyps_pad, hyps_lens, r_hyps_pad, self.reverse_weight
    ...
    def beam_search(
        ...
        """
        assert self.bs is not None, "bs is not initialized!"
        self.init_bs()
    ...
    def _compute_loss(
        ...
        if self.simple_lm_proj is None and self.simple_am_proj is None:
            ...
            assert self.simple_lm_proj is not None
            assert self.simple_am_proj is not None
            ...
            if steps < self.warmup_steps:
                simple_loss_scale = 1.0 - (steps / self.warmup_steps) * (1.0 - simple_loss_scale)
