...
class DecodeResult:
    ...
    def __init__(
        ...
        tokens_confidence: Optional[List[float]] = None,
        times: Optional[List[int]] = None,
        nbest: Optional[List[List[int]]] = None,
        nbest_scores: Optional[List[float]] = None,
        nbest_times: Optional[List[List[int]]] = None,
    ):
        """
        Args:
...
class PrefixScore:
    ...
    def __init__(
        ...
        self.times_s: List[int] = []  # times of viterbi blank path
        self.times_ns: List[int] = []  # times of viterbi none blank path
...
def ctc_prefix_beam_search(
    ...
    for i in range(batch_size):
        ...
        for t in range(0, num_t):
            ...
            next_hyps: defaultdict = defaultdict(lambda: PrefixScore())
            ...
            sorted_hyps = sorted(next_hyps.items(), key=lambda x: x[1].total_score(), reverse=True)
            cur_hyps = sorted_hyps[:beam_size]
...
def attention_beam_search(
    ...
    infos: Optional[Dict[str, List[str]]] = None,
) -> List[DecodeResult]:
    ...
    for i in range(prefix_len, maxlen + 1):
        ...
        base_cache_index = (
            torch.arange(batch_size, device=device).view(-1, 1).repeat([1, beam_size]) * beam_size
        ).view(
            -1
        )  # (B*N)
        ...
        cache["self_att_cache"] = {
            i_layer: (
                torch.index_select(value[0], dim=0, index=cache_index),
...
def attention_rescoring(
    ...
    for b in range(batch_size):
        ...
        if hyps is None or ctc_scores is None:
            # Skip if nbest results are not available
            continue
        ...
        for i, hyp in enumerate(hyps):
            ...
            if score > best_score:
                best_score = score.item() if hasattr(score, "item") else float(score)
                best_index = i
            tokens_confidences.append(tc)
        nbest_times = ctc_prefix_results[b].nbest_times
        results.append(
            DecodeResult(
                hyps[best_index],
                best_score,
                confidence=confidences[best_index],
                times=nbest_times[best_index] if nbest_times is not None else None,
