...
from torch.distributed.fsdp import MixedPrecision, ShardingStrategy, sharded_grad_scaler
from torch.nn.utils import clip_grad_norm_
from torch.utils.data import DataLoader

from chunkformer.utils.checkpoint import save_checkpoint
...
def add_model_args(parser):
    ...
    parser.add_argument(
        "--enc_init", default=None, type=str, help="Pre-trained model to initialize encoder"
    )
    parser.add_argument(
        "--enc_init_mods",
        default="encoder.",
...
def add_trace_args(parser):
    ...
    parser.add_argument("--print_model", action="store_true", default=False, help="print model")
    return parser


def add_dataset_args(parser):
    parser.add_argument(
        "--data_type", default="raw", choices=["raw", "shard"], help="train and cv data type"
    )
...
def add_lora_args(parser):
    ...
    """
    parser.add_argument(
        "--use_lora", default=False, type=bool, help="whether use the lora finetune."
    )
    parser.add_argument(
        "--only_optimize_lora",
        default=False,
    ...
    parser.add_argument("--lora_rank", default=8, type=int, help="lora rank num.")
    parser.add_argument(
        "--lora_alpha", default=8, type=int, help="lora scale param, scale=lora_alpha/lora_rank."
    )
    parser.add_argument("--lora_dropout", default=0, type=float, help="lora dropout param.")
    parser.add_argument("--lora_ckpt_path", default=None, type=str, help="lora checkpoint path.")
    parser.add_argument(
        "--lora_reinit",
        default=False,
        type=bool,
...
def add_deepspeed_args(parser):
    parser.add_argument(
        ...
        default=30,
        type=int,
        help="timeout (in seconds) of wenet_join. " + "30s for aishell & 300s for wenetspeech",
    )
    parser.add_argument(
...
def init_dataset_and_dataloader(args, configs, tokenizer, seed=777):
    ...
    cv_dataset = init_dataset(
        dataset_type, args.data_type, args.cv_data, tokenizer, conf, partition=False, split="cv"
    )
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        ...
        if int(os.environ.get("RANK", 0)) == 0:
            ...
            estimate_zero2_model_states_mem_needs_all_live(
                model, num_gpus_per_node=local_world_size, num_nodes=world_size // local_world_size
            )
        ...
        model = FSDP(
            ...
            cpu_offload=CPUOffload(offload_params=True) if args.fsdp_cpu_offload is True else None,
...
def log_per_step(writer, info_dict, timer: Optional[StepTimer] = None):
    ...
    if tag == "TRAIN" and rank == 0 and writer is not None:
        if (train_engine == "deepspeed" and is_gradient_accumulation_boundary) or (
            train_engine in ["torch_ddp", "torch_fsdp"] and (batch_idx + 1) % accum_grad == 0
        ):
            writer.add_scalar(
                "train/train_loss", tensor_to_scalar(loss_dict["loss"]) * accum_grad, step
            )
    ...
    if (batch_idx + 1) % log_interval == 0:
        ...
        log_str += "Batch {}/{} loss {:.6f} ".format(
            epoch,
            batch_idx + 1 if "save_interval" not in info_dict else (step + 1) * accum_grad,
...
def reinit_lora(model, args, configs, tokenizer, seed=777):
    ...
    dataset = init_dataset(
        dataset_type, args.data_type, args.train_data, tokenizer, dataset_conf, True
    )
    ...
    save_checkpoint(
        model, os.path.join(args.model_dir, "lora_init.pt"), infos={"tag": "lora_init", **configs}
    )
