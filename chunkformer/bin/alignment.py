...
def get_frames_timestamp(alignment, prob, blank_thres=0.999, thres=0.0000000001):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--blank_thres", default=0.999999, type=float, help="ctc blank thes")
    parser.add_argument("--thres", default=0.000001, type=float, help="ctc non blank thes")
    parser.add_argument("--checkpoint", required=True, help="checkpoint model")
    parser.add_argument("--dict", required=True, help="dict file")
    parser.add_argument("--non_lang_syms", help="non-linguistic symbol file. One symbol per line.")
    ...
    parser.add_argument("--bpe_model", default=None, type=str, help="bpe model for english part")
    ...
    ali_dataset = Dataset(args.data_type, args.input_file, tokenizer, ali_conf, partition=False)
    ...
    with torch.no_grad(), open(args.result_file, "w", encoding="utf-8") as fout:
        for batch_idx, batch in enumerate(ali_data_loader):
            ...
            if args.gen_praat:
                timestamp = get_frames_timestamp(alignment, ctc_probs, args.blank_thres, args.thres)
                subsample = get_subsample(configs)
                labformat = get_labformat(timestamp, subsample)

                lab_path = os.path.join(os.path.dirname(args.result_file), key[0] + ".lab")
