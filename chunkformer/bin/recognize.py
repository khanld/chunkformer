...
def get_args():
    ...
    parser.add_argument(
        "--search_ctc_weight", type=float, default=1.0, help="ctc weight for nbest generation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="transducer weight for rescoring weight in " "transducer attention rescore mode",
    )
    parser.add_argument(
        "--attn_weight",
        type=float,
    ...
    parser.add_argument("--word", default="", type=str, help="word file, only used for hlg decode")
    parser.add_argument("--hlg", default="", type=str, help="hlg file, only used for hlg decode")
    parser.add_argument(
        "--lm_scale", type=float, default=0.0, help="lm scale for hlg attention rescore decode"
    )
    parser.add_argument(
        "--decoder_scale", type=float, default=0.0, help="lm scale for hlg attention rescore decode"
    )
    parser.add_argument(
        "--r_decoder_scale",
        type=float,
    ...
    parser.add_argument(
        "--use_lora", type=bool, default=False, help="""Whether to use lora for biasing"""
    )
...
def main():
    ...
    test_dataset = Dataset(args.data_type, args.test_data, tokenizer, test_conf, partition=False)
